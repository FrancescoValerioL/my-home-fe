...
const ToDoList = () => {
	...
	const deleteItem = (idx: number) => {
		setItemsList((items) => {
			const updatedItems = [...items]; // Crea una copia dell'array originale
			updatedItems.splice(idx, 1); // Rimuovi l'elemento dalla copia
...

			{
				itemsList.map((item, idx) => (
					<Space.Compact key={idx} style={{ width: '100%' }}>
						...
						{itemsList[itemsList.length - 1] === item && <Button type="primary" icon={<PlusOutlined />} size="large" onClick={addNewItem}></Button>}
